...
## Usage
```
python main.py -u USERNAME -p PASSWORD -o OUTPUT url
```
- **To avoid credential leak, run the script without `-u`/`--user` and `p`/`--password` arguments and the script will ask the username and password interactively**.
- The url where the game is hosted can be passed to the script as positional argument.
